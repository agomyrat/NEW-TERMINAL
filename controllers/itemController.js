...
exports.search = catchAsync(async(req, res, next)=>{
    ...
    const text = await db.query(`SELECT TOP :limit 
...
exports.getByBarcode = catchAsync(async (req, res, next) => {
    ...
    if (!barcode) {
        res.status(400).json({ status: 'failed', message: 'Barcode is missing or worng' });
    }
    const info = await db.query(`
...
exports.getItemForSanaw = catchAsync(async (req, res, next) => {
    ...
    if (!barcode || !terminal) {
        res.status(400).json({status: 'failed',message:'Barcode or terminal is missing or worng'})
    }
    ...
    if (info[0]?.id > 0) {
        ...
        `, {
            replacements: { miktar: info[0].miktar, id:info[0].id, terminal },
...
exports.getAllSanaw = catchAsync(async (req, res, next) => {
    ...
    if (!terminal) {
        res.status(400).json({status: 'failed',message:'Terminal is missing or worng'})
    }
...
exports.changeCount = catchAsync(async (req, res, next) => {
    ...
    if (!terminal || !id) {
        res.status(400).json({ status: 'failed', message: 'Some parametrs are missing or worng' })
    }
    ...
    res.status(200).json({ status: "success", message: "Item count is updated" });
});
